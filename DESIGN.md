...
Airbnb's website is a warm, photography-forward marketplace that feels like flipping through a travel magazine where every page invites you to book. The design operates on a foundation of pure white (`#ffffff`) with the Sky Blue (`#3b82f6`) — a calm, trustworthy blue — serving as the singular brand accent. The result is a clean, airy canvas where listing photography, category icons, and the blue CTA button are the only sources of color.
...
**Key Characteristics:**
- Pure white canvas with Sky Blue (`#3b82f6`) as singular brand accent
...
### Primary Brand
- **Sky Blue** (`#3b82f6`): `--palette-bg-primary-core`, primary CTA, brand accent, active states
- **Deep Blue** (`#1d4ed8`): `--palette-bg-tertiary-core`, pressed/dark variant of brand blue
...
- Padding: 0px 24px
- Radius: 8px
- Hover: transitions to error/brand accent via `var(--accent-bg-blue)`
...
- Airbnb logo (Sky Blue) left-aligned
...
- Apply Sky Blue (`#3b82f6`) only for primary CTAs and brand moments — it's the singular accent
...
- Don't apply Sky Blue to backgrounds or large surfaces — it's an accent only
...
- Don't introduce additional brand colors beyond the Sky Blue/Luxe/Plus system
...
- "Design search bar: white background, full card shadow, 32px radius on container. Search text at 14px Cereal VF weight 400. Blue search button (#3b82f6, 50% radius, white icon)."
...
- "Create a CTA button: #222222 background, white text, 8px radius, 16px Cereal VF weight 500, 0px 24px padding. Hover: brand blue accent."
...
2. Sky Blue (#3b82f6) is the singular accent — use sparingly for CTAs only
